...
import { alertError, activeWorktreePath, baseWorktreePath, commitAll, confirmAction, hasPendingChanges, isBusy, listBranches, onBusyChange, onRepoChange, openIncomingDiff, openUrl, runPinned, samePath, toViewStatus, tryAction, } from "@/lib/git";
import { basename } from "@/lib/workspace";
import { openRepoPicker, renderRepoSwitcher } from "@/panel/repo-switcher";
...
export class GitPanelApp {
    ...
    start() {
        ...
        this.disposers = [
            ...
            muxy.events.subscribe("command.switch-repo", () => void openRepoPicker()),
            muxy.events.subscribe("project.switched", () => void this.updatePolling()),
            muxy.events.subscribe("worktree.switched", () => void this.updatePolling()),
            onRepoChange(() => {
                void this.switchScope();
                void this.resetGraph(false);
                this.reloadPrListOnScopeChange();
                void this.updatePolling();
            }),
    ...
    async updatePolling() {
        const [cwd, base] = await Promise.all([activeWorktreePath(), baseWorktreePath()]);
        const needsPoll = !!cwd && !!base && !samePath(cwd, base);
        if (needsPoll && !this.pollTimer) {
        ...
        else if (!needsPoll && this.pollTimer) {
    ...
    render() {
        ...
        if (this.repo.kind === "no_repo") {
            this.root.appendChild(h("div", { class: "flex h-screen flex-col" }, renderRepoSwitcher(this), emptyState(h("div", {}, "This folder is not a Git repository."), button("Initialize Repository", {
                variant: "outline",
        ...
        const panel = h("div", { class: "flex h-full min-h-0 flex-col" }, renderRepoSwitcher(this), h("header", { class: "flex shrink-0 items-center border-b border-border pr-1" }, h("div", { class: "min-w-0 flex-1" }, renderBranchSwitcher(this, status)), iconButton("Refresh", "refresh", () => this.runRefresh()), iconButton("Open repository in browser", "external", () => void this.openRepoInBrowser())), this.renderTabs(changes), this.tab === "branch"
    ...
    async loadLocal(withPr) {
        ...
        const cwd = await activeWorktreePath();
        this.repoName = cwd ? basename(cwd) : "";
        let next;
        try {
