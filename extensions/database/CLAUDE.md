...
- A pinned `connections` panel (`panel/connections.html` → `src/connections-panel.js`) lists saved connections. Clicking one emits `extension.database.open-connection`.
...
- The `workbench` tab (`panel/index.html` → `src/app.js` → `src/workbench/workbench.js`) is always opened with `muxy.data.connectionId` and hosts the schema browser, data grid, SQL editor, and structure views.
